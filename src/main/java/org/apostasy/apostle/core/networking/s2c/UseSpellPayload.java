 ...
 * @author Chemthunder
 */
public record UseSpellPayload(float size) implements CustomPayload {
    ...
    public static final PacketCodec<ByteBuf, UseSpellPayload> CODEC = PacketCodec.tuple(
            PacketCodecs.FLOAT, UseSpellPayload::size,
            UseSpellPayload::new
    );
    ...
    public static class Receiver implements ClientPlayNetworking.PlayPayloadHandler<UseSpellPayload> {
        public void receive(UseSpellPayload payload, ClientPlayNetworking.Context context) {
            context.client().execute(() -> SpellHudEvents.expansion = payload.size);
        }
    }
}
